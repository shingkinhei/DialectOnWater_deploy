...
function AudioRecorder(config) {
  ...
  function setupStorage() {
    ...
    if (Storage.ctx.createJavaScriptNode) {
      jsAudioNode = Storage.ctx.createJavaScriptNode(
        bufferSize,
        numberOfAudioChannels,
        numberOfAudioChannels
      );
    } else if (Storage.ctx.createScriptProcessor) {
      jsAudioNode = Storage.ctx.createScriptProcessor(
        bufferSize,
        numberOfAudioChannels,
        numberOfAudioChannels
      );
  ...
  function onMicrophoneCaptureError() {
    console.log(
      "There was an error accessing the microphone. You may need to allow the browser access"
    );
  }
