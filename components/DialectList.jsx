...
export default function DialectList({ id, dialect }) {
  ...
  return (
    <div
      className={
        currentUser?.role !== "admin" && dialect?.data()?.status === "pending"
          ? "hidden"
          : "relative dialect-list-card bg1 p-6 bg-white border-gray-200 rounded-lg flex flex-col justify-center shadow hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700 cursor-pointer"
      }
    >
      {dialect?.data()?.status === "pending" ? (
        <div className="absolute left-0 top-0">
          ...
          >
            <path
              d="M0 0H51V0C51 24.3005 31.3005 44 7 44H0V0Z"
              fill="#FFBB0C"
            />
        ...
        {playingStatus === "paused" ? (
          <a className="z-20" onClick={handlePlayPause}>
            <div className="rounded-full bg-gray-100 p-2 drop-shadow-lg">
              <BsPlayFill className="translate-x-1" size={80} color="#1D82BB" />
            </div>
          </a>
        ) : null}
      ...
      <div className="mt-2 flex justify-between items-center">
        ...
        {currentUser?.role === "admin" ? (
          <div className="z-30 border-l	pl-4">
            <a
              className="flex flex-row"
              onClick={() => router.push(`dialect/edit/${id}`)}
            >
              <span variant="h4" className="text-white">
                編輯
              </span>
